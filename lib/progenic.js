...
module.exports =
{
    run: function(options)
    {
        ...
        options.logger = utils.enrichLogger(options.logger);
        ...
        if (options.name && options.main)
        {
            ...
            options.checkPingsEnabled = options.checkPingsEnabled || true;
            ...
            process.on('SIGKILL', () =>
            {
                options.logger.warn('[PROGENIC] Got SIGKILL signal. Exiting...');
            });
            ...
            utils.createWorkers(options.workers, options.name, options.main, options.devMode, options.logger, options.checkPingsEnabled)
            .then( () =>
            {
