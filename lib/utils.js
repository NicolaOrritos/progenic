...
const helpers =
{
    ...
    restartIfDead: function(worker, options, log)
    {
        ...
        {
            log.info('[PROGENIC] Waiting the death of worker #%s...', worker.id);
            ...
            if (worker.isDead())
            {
                if (options && options.restart && options.name)
                {
                    ...
                    setTimeout( () => helpers.forkWorker(options.name, options.devMode, log, options.logsBasePath, options.checkPingsEnabled) , RESPAWN_WAIT_TIME);
                }
                else
                {
        ...
        if (!worker.isDead())
        {
            log.info('[PROGENIC] Killing it and waiting %s seconds for it to be dead for good...', retryInterval);
...
module.exports =
{
    ...
    enrichLogger: function(logger)
    {
        if (logger)
        {
            logger.trace = expandLoggingFn(logger.trace || module.exports.CONSOLE_LOGGER.trace);
            logger.debug = expandLoggingFn(logger.debug || module.exports.CONSOLE_LOGGER.debug);
            logger.info  = expandLoggingFn(logger.info  || module.exports.CONSOLE_LOGGER.info);
            logger.warn  = expandLoggingFn(logger.warn  || module.exports.CONSOLE_LOGGER.warn);
            logger.error = expandLoggingFn(logger.error || module.exports.CONSOLE_LOGGER.error);
            logger.fatal = expandLoggingFn(logger.fatal || module.exports.CONSOLE_LOGGER.fatal);
        }
