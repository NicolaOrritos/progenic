...
const helpers =
{
    ...
    restartIfDead: function(worker, options, log)
    {
        ...
        function waitForDeath()
        {
            ...
            {
                // Retry
                log.warn('After %s second worker "%s" is still alive. Waiting $s more second(s)...', retryInterval, worker.id, retryInterval);
            ...
            {
                log.fatal('Could not kill worker "%s"! Briging the whole process down!', worker.id);
