...
function countRespawns(log)
{
    ...
    log.info('[PROGENIC] Respawns have been "%s"', maxNumber);
...
const helpers =
{
        ...
        function waitForDeath()
        {
            if (worker.isDead())
            {
                if (options && options.restart && options.name)
                {
                    ...
                    setTimeout( () => helpers.forkWorker(options.name, options.devMode, log) , RESPAWN_WAIT_TIME);
                }
                else
                {
    ...
    forkWorker: function(name, devMode, log)
    {
        return new Promise( (resolve, reject) =>
        {
            if (name && log)
            {
                if (countRespawns(log) < MAX_RESPAWNS)
                {
                    ...
                    worker.on('exit', () =>
                    {
                        ...
                        .then( () =>
                        {
                            log.info('[PROGENIC] The new worker has been succesfully started');
                        })
        ...
        return new Promise( (resolve, reject) =>
        {
            if (cluster.isMaster)
            {
                ...
                while (count-- > 0)
                {
                    ...
                    promises.push(helpers.forkWorker(name, devMode, log));
                }
