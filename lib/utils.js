        ...
        return new Promise( (resolve, reject) =>
        {
            if (name && log)
            {
                if (respawns < MAX_RESPAWNS)
                {
                    ...
                    // Discard log file on errors and get a new one:
                    logFile.on('error', err =>
                    {
                        const filePath = helpers.buildLogFilePath(fileName, devMode, logsBasePath);

                        const newLogFile  = helpers.createLogFile(fileName, devMode, logsBasePath);

                        worker.process.stdout.pipe(newLogFile);
                        worker.process.stderr.pipe(newLogFile);

                        // Logging takes place after the reset of the log-file, because there was an error with the old one:
                        log.error('Error on write stream "%s". %s', filePath, err);
                        log.info('Discarded it and opened a new one...');
                    });
                    ...
                    worker.on('listening', () =>
                    {
                        log.info('Enabling check pings for worker #%s...', worker.id);
...
function expandLoggingFn(fn, thisArg)
{
    let result;
...
module.exports =
{
    ...
    normalizeOptions: function(options)
    {
        ...
        if (options)
        {
            ...
            result.logsBasePath      = options.logsBasePath;
    ...
    enrichLogger: function(logger)
    {
        if (logger)
        {
            logger.trace = expandLoggingFn(logger.trace || module.exports.CONSOLE_LOGGER.trace, logger);
            logger.debug = expandLoggingFn(logger.debug || module.exports.CONSOLE_LOGGER.debug, logger);
            logger.info  = expandLoggingFn(logger.info  || module.exports.CONSOLE_LOGGER.info,  logger);
            logger.warn  = expandLoggingFn(logger.warn  || module.exports.CONSOLE_LOGGER.warn,  logger);
            logger.error = expandLoggingFn(logger.error || module.exports.CONSOLE_LOGGER.error, logger);
            logger.fatal = expandLoggingFn(logger.fatal || module.exports.CONSOLE_LOGGER.fatal, logger);
        }
