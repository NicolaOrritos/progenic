    ...
    restartIfDead: function(worker, options, log)
    {
        ...
        function waitForDeath()
        {
            if (worker.isDead())
            {
                ...
                {
                    log.error('Could not restart worker #%s: missing information to do so', worker.id);
                }
            }
            else if (retries)
            {
                // Retry
                log.warn('After %s seconds worker #%s is still alive. Waiting %s more second(s)...', retryInterval, worker.id, retryInterval);
            ...
            {
                log.fatal('Could not kill worker #%s! Bringing the whole process down!', worker.id);
